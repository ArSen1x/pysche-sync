  ...
  return (
    <Dialog.Root open={!!code} onOpenChange={(open) => { if (!open) onClose() }}>
      <Dialog.Portal>
        ...
        {/* Centered panel */}
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 w-[92vw] max-w-md max-h-[85dvh] bg-white rounded-2xl shadow-xl flex flex-col overflow-hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95">

          {/* Close button */}
          <Dialog.Close asChild>
            <button className="absolute top-3 right-3 z-10 text-slate-400 hover:text-slate-600 transition-colors p-1 rounded-lg hover:bg-slate-100 [-webkit-tap-highlight-color:transparent]">
              <X size={16} />
            ...
            <div className="flex flex-col overflow-hidden">
              ...
              <div className="px-4 pt-4 pb-3 border-b border-slate-100">
                <div className="flex items-center gap-2 pr-7">
                  <span className="font-mono text-base font-bold text-sage-700 bg-sage-50 border border-sage-200 rounded-lg px-3 py-1">
                    {data.code}
                  </span>
                  ...
                  <button
                    ...
                    className="ml-auto shrink-0 p-1.5 rounded-lg hover:bg-slate-100 transition-colors [-webkit-tap-highlight-color:transparent]"
                  >
                    <Star
                      size={16}
                      className={isFavorited ? "fill-sage-500 text-sage-500" : "text-slate-300"}
                    />
                  </button>
                </div>

                <Dialog.Title className="text-sm font-semibold text-slate-800 leading-snug mt-2">
                  {data.description}
                </Dialog.Title>
                ...
                {data.parent && (
                  <Dialog.Description asChild>
                    <button
                      ...
                      className="mt-1.5 inline-flex items-center gap-1 text-xs text-sage-600 hover:text-sage-800 transition-colors cursor-pointer [-webkit-tap-highlight-color:transparent]"
                    >
              ...
              <div className="overflow-y-auto px-4 py-3 space-y-4">
                ...
                {/* DSM-5-TR Reference */}
                <section className="bg-blue-50 border border-blue-100 rounded-xl px-3 py-2.5">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {data.children.map((child) => (
                        <button
                          ...
                          className="inline-flex items-center gap-1 font-mono text-xs font-semibold text-sage-700 bg-sage-50 border border-sage-200 rounded-md px-2 py-1 hover:bg-sage-100 hover:border-sage-400 transition-colors active:scale-95 cursor-pointer [-webkit-tap-highlight-color:transparent]"
                        >
                          {child}
                ...
                {(data.excludes1.length > 0 || data.excludes2.length > 0) && (
                  <section>
                    ...
                    <Accordion type="multiple" className="border border-slate-100 rounded-xl overflow-hidden">
                      {data.excludes1.length > 0 && (
                        <AccordionItem value="excludes1">
                          <AccordionTrigger className="px-3 text-slate-700 font-medium">
                            Excludes 1
                          ...
                          <AccordionContent className="px-3">
                            <ul className="space-y-1.5">
                              {data.excludes1.map((note, i) => (
                                <li key={i} className="text-slate-600 text-xs leading-snug flex gap-2">
